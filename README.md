...
tinyply not what you're looking for? tinyply trades some performance for simplicity and flexibility. For domain-specific uses (e.g. where your application does not need to handle arbitrary user-fed PLY files), there may be other speedier alternatives. For more, please check out the following benchmarks:
...
* `version 3.0` is a performance optimization pass and adds support for variable length lists. 
